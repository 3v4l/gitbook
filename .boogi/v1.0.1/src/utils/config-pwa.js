...
const postProcessConfig = (config) => {
  ...
  manifest.short_name = calculateValue(
    manifest.short_name,
    shortNameFromMetadata(config.metadata)
  );
  manifest.description = calculateValue(
    manifest.description,
    config.metadata.description
  );
  manifest.start_url = calculateValue(
    manifest.start_url,
    config.metadata.pathPrefix
  );
  manifest.background_color = calculateValue(
    manifest.background_color,
    config.metadata.themeColor
  );
  manifest.theme_color = calculateValue(
